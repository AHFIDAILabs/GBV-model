...
def gbv_rule_insights(instance_data):
    ...
    if employment == 0:  # Assuming 0 = unemployed
        reasons.append("Unemployment can increase vulnerability to GBV by limiting financial independence, "
        "reducing access to protective resources, and creating dependency on potentially unsafe social networks.")
        suggestions.append( "Provide targeted economic support, vocational training, or income-generating programs to reduce financial dependency. "
        "Additionally, offer accessible counseling services and link the individual to local protection programs or safe spaces.")

    education = instance_data.get("educational_status")
    if education is not None and education <= 2:
        reasons.append("Low education level may limit awareness of GBV risks, reduce knowledge of rights and available support services, "
        "and hinder the ability to recognize and report abuse effectively.")
        suggestions.append("Implement targeted awareness and empowerment programs, including community workshops, informational campaigns, "
        "and literacy support. Ensure individuals are informed about their rights, reporting mechanisms, and available protective services.")
